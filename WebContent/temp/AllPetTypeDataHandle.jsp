...
<script>
	...
	$(function() {
		$("#getUrlSrc").click(function() {
							...
							$.ajax({
										...
										data : {
											...
											"StartPosition":"searchRight_body",
											"EndPosition":"</form>",
										},
										scriptCharset : 'utf-8',
		...
		$("#reSult").click(function() {
				...
				$("#saveFireshData").click(function() {
					var FireshNameData=[];
					var FireshPriceData=[];
					var FireshIMGPaht=[];
					var $Allli=$(".y_searchList").find("li");
					alert($Allli.length);
				
					/* alert($AllTr.eq(1).find("td").eq(0).text());获取名字*/
					for(var d=0;d<=$Allli.length;d++){
						FireshIMGPaht[d]=$Allli.eq(d).find("img").eq(0).attr("src");/*获取图片*/
						
						FireshNameData[d]=$Allli.eq(d).find("p[class='y_proName']").eq(0).find("a").text();/*获取名字*/
						
						FireshPriceData[d]=$Allli.eq(d).find("p[class='y_price']").eq(0).find("font").text();/*获取名字*/
					...
					$.ajax({
						...
						data : {
						 	...
						 	"FireshNameData":FireshNameData,
							"FireshPriceData":FireshPriceData,
							"FireshIMGPaht":FireshIMGPaht,
						},
						scriptCharset : 'utf-8',
						contentType : "application/x-www-form-urlencoded;charset=utf-8",
						dataType : "json",
...
<body>
	<div id="conent_control">
		...
		<s:textarea value="自动生成的图片地址" id="conent_ProudcutIMG_text" name="IMGUrlPath"></s:textarea>
		<div id="getData">
        <div id="getDataButton">
        <input id="getUrlSrc" value="获致源代码" type="button"></input>
         <input id="getIMGUrlSrc" value="获致IMG源代码" type="button"></input>
			...
			<input id="saveFireshData" value="保存所有蔬菜数据" type="button"></input>
			<!-- 数据分析 -->
