...
@WebServlet("/HRPayInfoAdd")
public class HRPayInfoAdd extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int resutl=0;
		String SendPath="";
		String payidStr=request.getParameter("paynumber");
		int payid=0;
		if(payidStr!=null){
		...
		try {
			resutl=daoInter.AddPayInfo(Payinfo);
		} catch (Exception e) {
			
			e.printStackTrace();
		}
		System.out.println("得到的结果是："+resutl);
		if(resutl!=0){
