...
@WebServlet("/SelectAllPayInfo")
public class SelectAllPayInfo extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int MaxPage=0;
		List<HRPayInfo> AllPersonInfolist=new ArrayList<HRPayInfo>();
		try {
			Maxcount=DaoInter.GetHRPayInfoMaxCount();
			...
			AllPersonInfolist=DaoInter.SelectAllPayInfo(thispage, rows);
		} catch (Exception e) {
			e.printStackTrace();
		}
		String ContextPath=request.getContextPath();
		session.setAttribute("AllPersonInfo", AllPersonInfolist);
		response.sendRedirect(ContextPath+"/HRPayManager/PayDataView.jsp");
